# Static QR generator (URL/Wi-Fi/vCard/batch) with SVG export, no signup, no redirect (`static-qr-no-signup`) — factory product contract

This repository is one product built by the Param Factory. It ships at
`https://static-qr-no-signup.sociobot.in`. Artifact class: `static-web`.
